...
// replace
import { fetchOne, update, create } from '../../service/InvoceOutService';
...
const InvoceOutItem = () => {
    ...
    const [item, setItem] = useState({'number': 0,'is_active':false, 'date':dayjs(), 'total':0,
            'invoice_out_list':[],'company':null, 'warehouse':null, 'partner':null,  'contract':null})
   ...
   const handleProcessRowUpdate =(newRow, oldRow) => {
        const updatedItems = item.invoice_out_list.map((itemList) => {
        ...
        setItem({...item, invoice_out_list: updatedItems, total: totalTab}); 
    ...
    function EditToolbar(props) {
        ...
        const { setItem } = props;
        const handleClick = () => {
            const updatedItems = item.invoice_out_list.map((itemList) => itemList);
            const number = updatedItems.length+1;
            updatedItems.push({ number:number, goods: null, price: 0, quantity: 0, sum:0, })

             setItem({...item, invoice_out_list: updatedItems}); 
    ...
    const handleDeleteClick = (id) => () => {
        const updatedItems = item.invoice_out_list.filter((row) => row.number !== id);
        let i=1;
        ...
        setItem({...item, invoice_out_list: newCountItems, total: totalTab}); 
    };
...
export default observer(InvoceOutItem);